...
typedef struct {
    uint8_t magic;         // Magic Number: set to $A8
    uint8_t version;       // Version: set to $01
    uint8_t frameDuration; // Frame Duration: number of 60Hz ticks between animation frames
    uint8_t mode;          // Mode: $00 = Text, $01 = Tile
    uint8_t columns;       // Columns: width of the animated area in tiles or characters
    uint8_t rows;          // Rows: height of the animated area in tiles or characters
    uint8_t xOffset;       // XOffset: X offset from upper left of the Tile Map or Text Display
    uint8_t yOffset;       // YOffset: Y offset from upper left of the Tile Map or Text Display
} FAOHeader;

typedef struct {
    uint8_t chunkType;   // Chunk Type: 1 byte
    uint8_t chunkID;     // Chunk ID: 1 byte
    uint16_t chunkLength; // Chunk Length: 2 bytes (little endian)
    // Chunk Data: of chunk length bytes
} FAOChunkHeader;
